...
Nearly one in three subscribers cancels, taking **$433,107 in monthly recurring revenue** with them — 29% of the total. That loss is not spread evenly. It concentrates in a profile defined by contract type, plan tier and acquisition channel — all recorded at signup — and it is preceded by a support ticket the business already logs. **Both signals are actionable before the customer is lost.**
...
| Metric | Value |
|---|---|
| Subscriber base | **100,000** customers · $1.49M MRR (~$17.9M ARR) |
| Churn rate | **32.68%** (32,676 customers) |
| Revenue at risk | **$433,107/mo — 28.98% of MRR** ($5.20M annualised) |
...
| Behavioural signal | Churn climbs 27.92% → 45.06% → 70.53% with support escalation |
...
The support table holds 18,718 tickets belonging to only 16,167 customers — 2,551 customers filed more than once. Joining on `customerid` caused these one-to-many matches to *fan out*, silently duplicating subscription records and inflating every downstream revenue and churn figure.
...
| 7 | ARPU | $14.95 |
| 8 | Total revenue & CLTV | $1,494,659 MRR · $31.18M CLTV · $311.82 avg |
| 9 | Revenue at risk | **$433,107/mo — 28.98% of MRR** |
| 10 | Average tenure | 642 days — retained 745 vs churned 430 |
| 11 | Complaints per user | 0.19 |
| 12 | Escalation rate | 7.78% of customers — **48.14% of tickets** |
| 13 | Support contact vs churn | 27.92% → 45.06% → 70.53% |
| 14 | Churn risk tiers | Low 18.92% · Medium 37.51% · High 62.84% |

Two definitions worth noting. **Revenue at risk** leads with share of MRR rather than a dollar total, because the percentage is what generalises. **Escalation rate** is reported on two denominators — across all customers it is 7.78%, but only 16,167 customers contacted support at all, so among actual tickets it is 48.14%. That second figure is the operational one.
...
Churn correlates most strongly with churn score (0.38), **contract type (−0.32)**, **lifetime value (−0.28)**, monthly charges (−0.24) and escalation status (0.23). CSAT (−0.17), plan tier (−0.18) and complaint volume (0.15) contribute in the expected directions. No single variable dominates — the drivers combine, which is what genuine behavioural data looks like.
...
The highest-leverage action available. Monthly contracts drive 62.1% of lost revenue while holding 38.4% of MRR. Prioritise the Basic-monthly cell at 60.09% churn. Against a ~51% monthly churn rate, an annual discount well below that threshold remains strongly economic.
...
Churn rises sharply the moment a ticket is filed (27.92% → 45.06%) and reaches 70.53% on escalation. Waiting for escalation means waiting for the situation to deteriorate. Route tickets from high-value or high-risk customers into a retention queue in parallel with normal support handling.
...
**48.14% of support tickets require escalation** — an operational problem independent of churn that feeds directly into the highest-risk cohort. Establish whether the driver is product defect, policy friction, or agent authority, and track escalation rate as a retention metric rather than solely a support metric.
...
**Correlation on binary variables.** The escalation–churn correlation of 0.23 understates the relationship, as Pearson correlation between two binary variables is bounded by their base rates. The support ladder (27.92% → 45.06% → 70.53%) is the more faithful presentation of the same finding and is what the notebook leads with.
